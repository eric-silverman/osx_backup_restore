...
timed "Restoring Editor Settings (VS Code, Cursor, Sublime)" '
  ...
  if [ -d "$FILES_DIR/xcode_user_data" ]; then
    mkdir -p ~/Library/Developer/Xcode/UserData
    rs_quiet "$FILES_DIR/xcode_user_data/" ~/Library/Developer/Xcode/UserData/
    note "Xcode user data (snippets, key bindings, themes) restored."
  fi
'
...
timed "Restoring LaunchAgents & cron" '
  ...
  # Prefer the explicit files/launch_agents_user copy; fall back to User_Folder sweep
  if [ -d "$FILES_DIR/launch_agents_user" ]; then
    mkdir -p ~/Library/LaunchAgents
    rs_quiet "$FILES_DIR/launch_agents_user/" ~/Library/LaunchAgents/
    note "User LaunchAgents restored."
  elif [ -d "$BACKUP_DIR/User_Folder/Library/LaunchAgents" ]; then
    mkdir -p ~/Library/LaunchAgents
    rs_quiet "$BACKUP_DIR/User_Folder/Library/LaunchAgents/" ~/Library/LaunchAgents/
    note "LaunchAgents restored from User_Folder."
  fi
'

# ====== sudoers Drop-ins ======
timed "Restoring sudoers Drop-ins" '
  if [ -d "$FILES_DIR/sudoers_d" ] && [ "$(ls -A "$FILES_DIR/sudoers_d" 2>/dev/null)" ]; then
    note "sudoers drop-ins copied to ~/Desktop/sudoers_d_from_backup/ — review and apply manually."
    cp -r "$FILES_DIR/sudoers_d" ~/Desktop/sudoers_d_from_backup
    note "To apply: sudo cp ~/Desktop/sudoers_d_from_backup/<file> /etc/sudoers.d/ && sudo chmod 440 /etc/sudoers.d/<file>"
  else
    note "No sudoers drop-ins in backup (skipping)."
  fi
'

# ====== System Network Config ======
timed "Restoring System Network Config" '
  if [ -d "$FILES_DIR/network_config_system" ] && [ "$(ls -A "$FILES_DIR/network_config_system" 2>/dev/null)" ]; then
    note "System network config size: $(size_of "$FILES_DIR/network_config_system")"
    copy_sys "$FILES_DIR/network_config_system/" "/Library/Preferences/SystemConfiguration/"
    note "Network config restored. A reboot is recommended for changes to take effect."
  else
    note "No system network config in backup (skipping)."
  fi
'

# ====== Keyboard Layouts & Input Methods ======
timed "Restoring Keyboard Layouts & Input Methods" '
  if [ -d "$FILES_DIR/keyboard_layouts_user" ]; then
    mkdir -p ~/Library/Keyboard\ Layouts
    rs_quiet "$FILES_DIR/keyboard_layouts_user/" ~/Library/Keyboard\ Layouts/
    note "User keyboard layouts restored."
  fi
  if [ -d "$FILES_DIR/keyboard_layouts_system" ]; then
    note "System keyboard layouts: $(size_of "$FILES_DIR/keyboard_layouts_system")"
    copy_sys "$FILES_DIR/keyboard_layouts_system/" "/Library/Keyboard Layouts/"
  fi
  if [ -d "$FILES_DIR/input_methods" ]; then
    mkdir -p ~/Library/Input\ Methods
    rs_quiet "$FILES_DIR/input_methods/" ~/Library/Input\ Methods/
    note "Input methods restored."
  fi
'

# ====== /etc/hosts ======
timed "Restoring /etc/hosts" '
  if [ -f "$FILES_DIR/etc_hosts" ]; then
    note "Backed-up /etc/hosts copied to ~/Desktop/etc_hosts_from_backup.txt — review and merge manually."
    cp "$FILES_DIR/etc_hosts" ~/Desktop/etc_hosts_from_backup.txt
    note "Diff of current vs backed-up /etc/hosts:"
    diff /etc/hosts "$FILES_DIR/etc_hosts" || true
  else
    note "No /etc/hosts in backup (skipping)."
  fi
'

# ====== Printer PPDs ======
timed "Restoring Printer PPDs" '
  if [ -d "$FILES_DIR/cups_ppd" ] && [ "$(ls -A "$FILES_DIR/cups_ppd" 2>/dev/null)" ]; then
    note "Printer PPDs size: $(size_of "$FILES_DIR/cups_ppd")"
    copy_sys "$FILES_DIR/cups_ppd/" "/etc/cups/ppd/"
    note "Printer PPDs restored. Re-add printers in System Settings > Printers & Scanners."
    if [ -f "$BACKUP_DIR/lists/printers.txt" ]; then
      cp "$BACKUP_DIR/lists/printers.txt" ~/Desktop/printers_from_backup.txt
      note "Printer list saved to ~/Desktop/printers_from_backup.txt for reference."
    fi
  else
    note "No printer PPDs in backup (skipping)."
  fi
'
